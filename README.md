...
- 贡献是**阶梯**：评论 → 行内批注 → 编辑建议 → 修订申请 → 直接修订，随信任等级逐级解锁；
- 治理是**双线**：自上而下的职务角色（超管/管理员/板块版主/编辑）与自下而上的信任等级并行；
- 发布有**审批**：所有文章经审批后公开，已发布文章的修改按页面模式（私有/公共）与信任等级走直接修订（进巡查）、修订申请或编辑建议；管理员可锁定编辑。
...
五个架构里程碑（M0–M5）已全部交付，现处打磨与硬化阶段（详见 `docs/02-architecture.md` §8）；M5 后的模型调整见 ADR-0007～0011。结构性变更（数据模型、权限模型、核心选型）合入前必须附 [ADR](docs/adr/README.md)。
