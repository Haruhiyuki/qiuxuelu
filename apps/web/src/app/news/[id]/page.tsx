...
import { getPublishedAnnouncement } from '@/server/announcements';
...
export async function generateMetadata({ params }: NewsDetailProps): Promise<Metadata> {
  ...
  const description = item.body.replaceAll('\n', ' ').trim().slice(0, 120);
  return { title: `${item.title} · 近闻`, description };
}
