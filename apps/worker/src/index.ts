// harublog worker：消费 search_outbox（事务性 outbox 模式），把发布/下线事件同步进 Meilisearch。
// 设计取舍：outbox 表本身就是持久可重放的队列，故直接轮询而非引入 pg-boss——
// pg-boss 留给 M2 的真·异步作业（邮件摘要、信任结算）。索引可随时从 PG 全量重建（见 reindex.ts）。
