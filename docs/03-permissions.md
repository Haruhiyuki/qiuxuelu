...
> 关联：ADR-0005（双线权限）、ADR-0007（页面模式）、ADR-0008（作者完整协作权）、ADR-0009（AI 评论审核）、ADR-0010（协作三件套重构）、ADR-0011（编辑策略二元化）、ADR-0012（实时协作退役）、ADR-0013（公共化阈值）、ADR-0016（信任晋升改贡献积分制）。
> 本文是**活文档**（随代码更新）；ADR 是某次决策的时点记录。信任晋升口径以 ADR-0016（积分制）为准。
