...
- **自动**：私有页累计协作 ≥ 阈值即升级。阈值入 `site_settings['doc.publicize'].threshold`（**不硬编码**，遵守治理阈值红线）。检查点 `maybeAutoPromote` 挂在「可能改变计数」的写路径后，廉价幂等、失败不连累主流程。
  - **⚠️ 口径与默认阈值已被 ADR-0013 取代**：现口径 = 非作者的（被采纳的修订申请 `status='merged'` + 他人署名的主线直编修订，去重 merge commit）之和，默认阈值 50；评论 / 编辑建议 / 未采纳的修订申请**不计**。本段原口径（建议 + 评论 + 修订、默认 20）仅作历史记录。
