...
> 状态：已采纳（2026-06-10）。
> 形成过程：三份独立架构提案（协作内核优先 / 产品体验优先 / 治理权限优先）+ 两份调研报告 + 三位评委交叉评审；三位评委一致裁定以提案 A 为骨架，嫁接 C 的治理层与 B 的阅读端方案，并补齐三案共同盲区。（原始材料为设计过程归档，不随公开仓库分发。）
> 本文是唯一权威架构文档；与提案原文冲突处以本文为准。
> **现状校正（以 ADR 为准）**：本文为 M0 定稿基线，此后产品/模型调整均以 ADR 记录并优先于本文——实时协作已退役、并发统一走修订层三方合并（ADR-0012）；信任晋升改为贡献积分制（ADR-0016）；内容许可改 CC BY-NC-SA（ADR-0015）；另见 ADR-0007/0008/0009/0010/0011/0013/0014。
...
| **M4 实时协作** ✅ | Hocuspocus 网关、checkpoint 缝合、presence | 断网重连不丢字；Y.Doc 与修订可互相重建 |
| **M5 规模化** ✅ | 语义搜索（可插拔 embedder）、数据导出（全站内容 CC 协议开放 dump）、透明度报告、备份演练 | RTO<1h，RPO<5min（恢复演练已实跑通过） |
