...
M5 后的产品/模型调整均以 ADR 记录、勿凭旧印象：ADR-0007 页面模式（私有/公共）、ADR-0008 作者完整协作权（TL0 起）、ADR-0009 AI 评论审核取代预审/限速（TL0 即可评论）、ADR-0010 协作三件套（修订/修订申请/编辑建议）改名与重构、ADR-0011 编辑策略简化为二元（锁定/开放）、ADR-0012 退役实时协作并把并发统一到修订层三方合并、ADR-0013 私有→公共升级口径收紧为「实质协作」（非作者：被采纳的修订申请 status=merged + 主线直编修订，去重 merge commit；评论/编辑建议/未采纳申请不计）+ 默认阈值 50（仍可 site_settings 覆盖）。另已落地：点赞/收藏、板块邮件订阅、OG 分享卡、响应式图片、阅读时长/目录滚动高亮、个人中心、协作公示页。
