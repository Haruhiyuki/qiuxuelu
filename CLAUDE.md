...
M5 后的产品/模型调整均以 ADR 记录、勿凭旧印象：ADR-0007 页面模式（私有/公共）、ADR-0008 作者完整协作权（TL0 起）、ADR-0009 AI 评论审核取代预审/限速（TL0 即可评论）、ADR-0010 协作三件套（修订/修订申请/编辑建议）改名与重构、ADR-0011 编辑策略简化为二元（锁定/开放）、ADR-0012 退役实时协作并把并发统一到修订层三方合并。另已落地：点赞/收藏、板块邮件订阅、OG 分享卡、响应式图片、阅读时长/目录滚动高亮、个人中心、协作公示页。
